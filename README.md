...
请关闭托管平台的请求体日志与敏感数据采集。本站不记录供应商返回的原始邮箱、Account ID、完整 Key 或卡密，不返回 GPT 完整 Account ID 或原始邮箱。上线前仍需使用供应商授权的测试卡与实际 Key 完成端到端验收；本地模拟测试不代表生产连通。不要把 Key 写入 `.env` 后提交到 Git。
...
个人工作台已拆分到独立项目 `tim-recharge-workbench`。本仓库仅保留客户自助充值及既有合作方接口，不提供 `/admin` 或管理 API，也不连接工作台数据库。两套系统不共享库存或锁；已发出的卡不得继续作为工作台可用库存。
