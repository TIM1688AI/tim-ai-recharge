...
Claude 充值记录另通过文档中的 `POST /api/agent/v1/cards/query` 查询同一卡密的 `account_id`，按站长确认的字段含义将其作为 Organization ID。仅向持有对应 TIMC 卡密的查询者返回经 UUID 校验的 `organization_id`，并与原充值状态核对；没有可靠结果时显示“暂未获取”，不影响充值状态展示。查询不持久化组织 ID，不返回供应商邮箱或完整原始身份响应；GPT 记录不新增身份查询。请妥善保管完整卡密，拥有卡密的人可以查询相关记录及组织 ID。

请关闭托管平台的请求体日志与敏感数据采集。本站不记录供应商返回的原始邮箱、Account ID、完整 Key 或卡密，不返回 GPT Account ID 或原始邮箱。上线前仍需使用供应商授权的测试卡与实际 Key 完成端到端验收；本地模拟测试不代表生产连通。不要把 Key 写入 `.env` 后提交到 Git。

## 合作方 API（默认关闭）

新增邀请制 `/partner-api/v1/*`，支持独立 Key、通道权限、持卡调用、限流、Upstash Redis 持久化请求去重和安全结果映射。网页功能不变。配置、调用契约及第一版限制见 [PARTNER_API.md](PARTNER_API.md)。这部分不依赖 Render 持久化磁盘，但必须配置 Upstash REST URL、Token 和稳定的摘要密钥，不能用网页的临时内存保护替代。不要在供应商授权与部署验收前开启。
