...
test('advanced display distinguishes unused, missing and uncertain results', () => {
  const { getTaskStatus } = require('./app');
  for (const [status, kind] of [['active', 'unused'], ['used', 'unconfirmed'], ['unknown', 'unconfirmed'], ['error', 'unconfirmed'], ['pending', 'processing'], ['success', 'completed'], ['failed', 'failed']]) {
    assert.equal(getTaskStatus(taskResult({ status }, 'JZ-ABCDEFGHIJK')).kind, kind);
  }
  assert.equal(getTaskStatus(taskResult({ found: false, status: 'error' }, 'JZ-ABCDEFGHIJK')).kind, 'missing');
